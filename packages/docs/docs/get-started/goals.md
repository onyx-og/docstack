...
* **Maturing cross-instance sync.** `@docstack/client` now owns a first-class, transport-agnostic sync layer — `stack.sync({ remote })` — with **Google Drive as its first supported transport**, so an application gets multi-device sync and backup without running a server or holding user data (see [Sync & backup](../sync/overview.md)). Firestore and peer-to-peer transports are the next ones planned against the same lifecycle: a different remote factory, the same call. The older, server-side `ReplicationService` that batched changes to a CouchDB-compatible endpoint (prototyped against IBM Cloudant) remains a separate, still-experimental path (see [Communication](../architecture/communication.md)); folding it onto the client's lifecycle is an open item. Remaining work on the client layer: an end-to-end two-device run against real Google Drive, and application-level conflict resolution beyond PouchDB's deterministic winner.
...
* **First-class multi-node deployments** where several DocStack instances (browser, server, mobile) stay consistently synced, building on the sync work above, so the "everything is a document" model extends cleanly across a distributed set of devices, not just a single database. With Google Drive covering the personal multi-device case today, the gap is shared workspaces — a team transport against the same `stack.sync()` lifecycle.
