...
**Serverless personal apps with user-owned backup.** Because a stack can sync to the user's **own Google Drive**, an application can offer multi-device sync and real backup with no server, no storage bill, and no custody of anyone's data — the files live in the user's Drive, under their quota and their control, with encrypted attributes unreadable to Google. This suits personal tools, journals, note-taking, habit and finance trackers, and anything where "we don't hold your data" is a feature rather than a compromise. See [Sync & backup](../sync/overview.md).
