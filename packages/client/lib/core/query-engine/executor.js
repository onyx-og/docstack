...
async function executeSingleSelectPlan(stack, plan, params, outerRow, outerAliases) {
    // 1. Fetch from table
    const fromClass = await stack.getClass(plan.fromTable.table);
    if (!fromClass)
        throw new Error(`Table not found: ${plan.fromTable.table}`);
    ...
    for (const join of plan.joins) {
        ...
        if (!rightClass)
            throw new Error(`Table not found: ${join.table}`);
        ...
        const finalRightSelector = Object.assign(Object.assign({}, rightBaseSelector), subqueryFilterSelector);
        ...
        for (const leftRow of currentResultRows) {
            ...
            if (join.on.operator === 'IN' && Array.isArray(fk_list)) {
                for (const fk of fk_list) {
                    if (rightRowMap.has(fk)) {
                        newResultRows.push(Object.assign(Object.assign({}, leftRow), { [joinAlias]: rightRowMap.get(fk) }));
            ...
            if (!matchFoundForLeftRow && join.type === 'LEFT') {
                ...
                newResultRows.push(Object.assign(Object.assign({}, leftRow), nullRightRow));
            }
        }
        if (join.type === 'RIGHT') {
            for (const rightRow of allRightRows) {
                if (!matchedRightIds.has(rightRow._id)) {
                    ...
                    newResultRows.push(Object.assign(Object.assign({}, nullLeftRow), { [joinAlias]: rightRow }));
                }
            }
        }
    ...
    if (plan.aggregation) {
        const fromAlias = plan.fromTable.as || plan.fromTable.table;
        const joinAliases = plan.joins.reduce((acc, j) => (Object.assign(Object.assign({}, acc), { [j.as || j.table]: j.table })), {});
...
async function executeUnionPlan(stack, plan, params) {
    if (plan.selectPlans.length === 0)
        return [];
    // Step 1: Execute all SELECT sub-queries to get their result sets.
    // This can be done in parallel for efficiency.
    const allSelectResults = await Promise.all(plan.selectPlans.map(p => executeSingleSelectPlan(stack, p, params)));
 ...
 */
export async function executePlan(stack, plan, params, outerRow = null, outerAliases = null) {
    if (!plan)
        return [];
    if (plan.type === 'union') {
