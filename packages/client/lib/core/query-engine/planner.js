...
function findExpressionRefs(expr, refs) {
    ...
    if (expr.left)
        findExpressionRefs(expr.left, refs);
    if (expr.right)
        findExpressionRefs(expr.right, refs);
    if (expr.expr)
        findExpressionRefs(expr.expr, refs);
    if (expr.args && expr.args.expr)
        findExpressionRefs(expr.args.expr, refs);
    if (Array.isArray(expr.exprs))
        expr.exprs.forEach(e => findExpressionRefs(e, refs));
}
/**
...
function findCorrelation(subqueryAst, outerTableAliases) {
    ...
    function traverse(predicate) {
        ...
        if (predicate.left)
            traverse(predicate.left);
        if (predicate.right)
            traverse(predicate.right);
    }
    ...
    if (correlations.length > 0) {
        ...
        function filterWhere(predicate) {
            if (!predicate || correlationSet.has(predicate))
                return null;
            if (predicate.type === 'and') {
                const newLeft = filterWhere(predicate.left);
                const newRight = filterWhere(predicate.right);
                if (newLeft && newRight)
                    return Object.assign(Object.assign({}, predicate), { left: newLeft, right: newRight });
...
function decorrelateSubqueries(ast) {
    ...
    function transformPredicate(predicate) {
        ...
        if (predicate.left)
            predicate.left = transformPredicate(predicate.left);
        if (predicate.right)
            predicate.right = transformPredicate(predicate.right);
...
function createSingleSelectPlan(ast) {
    ...
    for (const join of ast.joins) {
        plan.joins.push(Object.assign(Object.assign({}, join), { filters: join.filters || [] }));
    }
    ...
    for (const predicate of wherePredicates) {
        ...
        if (isSimple && referenced.length === 1 && referenced[0] === fromTableName) {
            ...
            if (!plan.filters.right[join.table])
                plan.filters.right[join.table] = [];
