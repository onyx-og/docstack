...
const testDataModel = async () => {
    ...
    // Store.clear("db-test")
    // return;
    let testStore = await Store.create("db-test", {plugins: []});
    console.log("testDataModel -  built store instance. Name: ", testStore.getDbName())
    var TestClass = await Class.create(testStore, "TestClass", "class", "A test");
    ...
    UserClass = await testStore.getClass("User");
    ...
    var UserSessionClass = await Class.create(testStore, "UserSession", "class", "Tracks user sessions");
    ...
    // reinstantiate UserClass from db
    UserSessionClass = await testStore.getClass("UserSession");
...
// FAILS
// var UserClass = new Class(testStore, "User", "class", "A user class for secure login");
